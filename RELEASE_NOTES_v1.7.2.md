...
### `search_annotations`
搜索 annotations 文本。Annotations 是后期加在 memory 上的注释 / 修正。这个 tool 让你按"被加了什么"找 memory，不是按 memory 自身内容找。

### `cite_memory`
显式给 memory 加 +1 usage_count。多数 retrieval 自动 cite，但当你不通过 search 直接用 memory 内容时（比如 context 里已经有了），用这个标记一下让 citation 计数准确。
...
修了:
- MCP server 现在 expose wakeup + 3 个 comment 工具 + pin/unpin + search_annotations + cite_memory
- serverInfo version 从 1.0.0 bump 到 1.7.2
- Tool 总数从 13 → 21
- README 的 wakeup + comments section 加了 MCP usage 示例
