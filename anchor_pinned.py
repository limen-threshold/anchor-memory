...
_RESERVED_FILES = {SESSION_STATE, RECENT_TIMELINE, LAST_SESSION, ORDER_MANIFEST}
...
def _check_identity_name(name: str) -> str:
    ...
    if name in _RESERVED_FILES:
        raise ValueError(f"{name} is maintained by Anchor itself (use write_session_state for session_state.md)")
    return name


def list_identity_files(pinned_dir: str) -> list:
    """Every identity file wakeup() would load, with size, mtime, and whether the _order.txt
    manifest (if any) includes it — a file missing from an existing manifest is silently skipped
    by load_pinned(), so the caller needs to know."""
    ...
    for n in sorted(os.listdir(pinned_dir)):
        ...
        out.append({"name": n, "chars": len(read_file(pinned_dir, n)),
                    ...
                    "loaded_by_wakeup": (order is None) or (n in order),
                    "kind": "session_state" if n == SESSION_STATE else ("timeline" if n == RECENT_TIMELINE else "identity")})
    return out
