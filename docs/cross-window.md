...
**Identity files in MCP-only setups.** `wakeup()` returns every `*.md` in the pinned directory (default `<db-path>/pinned/`, all files except `last_session.md`, in `_order.txt` order or alphabetical). That is where a self-description file lives — call it `identity.md`, `我是谁.md`, whatever you like; the name is yours, Anchor only cares that it is a `.md` in that folder. How it differs from memories: a memory is a fragment that has to win a search (or be pinned) to come back; an identity file is a whole first-person text that comes back every time, unranked, verbatim. In a hosted client both arrive through the same `wakeup()` tool result, so the difference is in *what* they are, not in *how* they are delivered. One honest gap: over MCP there is no tool that writes arbitrary pinned files — `write_session_state` only writes `session_state.md`. So in a Chat-only setup the identity file is maintained by whoever has file access to that folder (a Claude Code session, a script, a person typing what the model dictates), or you keep the self-description inside `session_state.md` for now, knowing that file is rolling state and gets rewritten.
