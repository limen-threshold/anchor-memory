...
"Want to do something small together?" — one card a day from a pool that is the AI's own: `<pinned>/invitations.md`, empty by default, filled by the AI with `invitation_add` from anything it remembers about the person — "you like pink, so write your name in a pink marker" — small and about them, not complex (starters in `docs/` are opt-in, never applied). The AI may offer the card or not; the person may do it or not. If they do, `invitation_done(name, note)` stores the moment as a **named** shared memory (tag `together`) — the reward is the name, not points. `wakeup()` and the proxy hand the card over once a day. No streaks, no scores, no GPS. Details: [docs/release-notes/v1.15.md](docs/release-notes/v1.15.md).
